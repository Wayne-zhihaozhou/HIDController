...
// Returns false with a Python exception set when obj is neither a valid button
// index nor a recognised button name.
static bool py_to_mouse_button(PyObject* obj, MouseButton& out) {
    if (PyUnicode_Check(obj)) {
        const char* s = PyUnicode_AsUTF8(obj);
        if (!s) return false;
        if (strcmp(s, "left") == 0)       { out = MouseButton::LEFT;   return true; }
        if (strcmp(s, "right") == 0)      { out = MouseButton::RIGHT;  return true; }
        if (strcmp(s, "middle") == 0)     { out = MouseButton::MIDDLE; return true; }
        if (strcmp(s, "xbutton1") == 0)   { out = MouseButton::X1;     return true; }
        if (strcmp(s, "xbutton2") == 0)   { out = MouseButton::X2;     return true; }
        if (strcmp(s, "x1") == 0)         { out = MouseButton::X1;     return true; }
        if (strcmp(s, "x2") == 0)         { out = MouseButton::X2;     return true; }
        PyErr_Format(PyExc_ValueError, "unknown mouse button name: '%s'", s);
        return false;
    }
    ...
    switch (val) {
        ...
        case 2: out = MouseButton::RIGHT;  return true;
        case 3: out = MouseButton::MIDDLE; return true;
        case 4: out = MouseButton::X1;     return true;
        case 5: out = MouseButton::X2;     return true;
    }
    PyErr_Format(PyExc_ValueError, "mouse button out of range (expected 1-5): %ld", val);
    return false;
}
...
// Returns false with a Python exception set when vk_obj is neither a usable
// integer usage ID nor a recognised key name / single character.
static bool py_to_vk(PyObject* vk_obj, KeyCode& out) {
    if (PyUnicode_Check(vk_obj)) {
        Py_ssize_t len;
        const char* s = PyUnicode_AsUTF8AndSize(vk_obj, &len);
        if (!s) return false;
        KeyCode code = key_name_to_vk(s);
        if (static_cast<uint16_t>(code) != 0) { out = code; return true; }
        // Single character fallback: ASCII → USB HID Usage ID
        if (len == 1) {
            uint16_t usage = ascii_to_usb(s[0]);
            if (usage != 0) { out = static_cast<KeyCode>(usage); return true; }
        }
        PyErr_Format(PyExc_ValueError, "unknown key name: '%s'", s);
        return false;
    }
    long val = PyLong_AsLong(vk_obj);
    if (val == -1 && PyErr_Occurred()) return false;
    if (val < 0 || val > 0xFFFF) {
        PyErr_Format(PyExc_ValueError, "key code out of range (expected 0-65535): %ld", val);
        return false;
    }
    out = static_cast<KeyCode>((uint16_t)val);
    return true;
}
...
static PyObject* hid_mouse_button_op(PyObject* self, PyObject* args, int op) {
    ...
    MouseButton btn;
    if (!py_to_mouse_button(btn_obj, btn))
        return NULL;
    bool result;
...
static PyObject* hid_key_sequence_op(PyObject* self, PyObject* args, bool sequential) {
    ...
    for (Py_ssize_t i = 0; i < n; i++) {
        ...
        if (!item) return NULL;
        KeyCode code;
        if (!py_to_vk(item, code)) return NULL;
...
static PyObject* hid_begin_key_intercept(PyObject* self, PyObject* args) {
    begin_key_intercept();
    Py_RETURN_NONE;
}

static PyObject* hid_end_key_intercept(PyObject* self, PyObject* args) {
    end_key_intercept();
    Py_RETURN_NONE;
}

static PyObject* hid_discard_queued_keys(PyObject* self, PyObject* args) {
    discard_queued_keys();
    Py_RETURN_NONE;
}
...
static PyObject* hid_get_key_code_map(PyObject* self, PyObject* args) {
    ...
    KEY_CODE_ENTRY(CANCEL);    KEY_CODE_ENTRY(BACK);
    KEY_CODE_ENTRY(TAB);       KEY_CODE_ENTRY(ENTER);
    ...
    KEY_CODE_ENTRY(DOWN);      KEY_CODE_ENTRY(SNAPSHOT);
    ...
    KEY_CODE_ENTRY(ADD);       KEY_CODE_ENTRY(SUBTRACT);
    KEY_CODE_ENTRY(DECIMAL);   KEY_CODE_ENTRY(DIVIDE);
    KEY_CODE_ENTRY(F1);        KEY_CODE_ENTRY(F2);
    KEY_CODE_ENTRY(F3);        KEY_CODE_ENTRY(F4);
    KEY_CODE_ENTRY(F5);        KEY_CODE_ENTRY(F6);
    KEY_CODE_ENTRY(F7);        KEY_CODE_ENTRY(F8);
    KEY_CODE_ENTRY(F9);        KEY_CODE_ENTRY(F10);
    KEY_CODE_ENTRY(F11);       KEY_CODE_ENTRY(F12);
    KEY_CODE_ENTRY(F13);       KEY_CODE_ENTRY(F14);
    KEY_CODE_ENTRY(F15);       KEY_CODE_ENTRY(F16);
    KEY_CODE_ENTRY(F17);       KEY_CODE_ENTRY(F18);
    KEY_CODE_ENTRY(F19);       KEY_CODE_ENTRY(F20);
    KEY_CODE_ENTRY(F21);       KEY_CODE_ENTRY(F22);
    KEY_CODE_ENTRY(F23);       KEY_CODE_ENTRY(F24);
    KEY_CODE_ENTRY(NUM_LOCK);  KEY_CODE_ENTRY(SCROLL_LOCK);
    KEY_CODE_ENTRY(LSHIFT);    KEY_CODE_ENTRY(RSHIFT);
    KEY_CODE_ENTRY(LCTRL);     KEY_CODE_ENTRY(RCTRL);
    KEY_CODE_ENTRY(LALT);      KEY_CODE_ENTRY(RALT);
    KEY_CODE_ENTRY(OEM_1);     KEY_CODE_ENTRY(OEM_PLUS);
    KEY_CODE_ENTRY(OEM_COMMA); KEY_CODE_ENTRY(OEM_MINUS);
    KEY_CODE_ENTRY(OEM_PERIOD); KEY_CODE_ENTRY(OEM_2);
    KEY_CODE_ENTRY(OEM_3);     KEY_CODE_ENTRY(OEM_4);
    KEY_CODE_ENTRY(OEM_5);     KEY_CODE_ENTRY(OEM_6);
    KEY_CODE_ENTRY(OEM_7);     KEY_CODE_ENTRY(OEM_102);
...
static PyMethodDef HidSendMethods[] = {
    ...
    {"begin_key_intercept",    (PyCFunction)hid_begin_key_intercept,    METH_NOARGS,
     "Start swallowing physical keystrokes into a queue (max 512 entries)."},
    {"end_key_intercept",      (PyCFunction)hid_end_key_intercept,      METH_NOARGS,
     "Stop intercepting and replay the queued keystrokes in order."},
    {"discard_queued_keys",    (PyCFunction)hid_discard_queued_keys,    METH_NOARGS,
     "Stop intercepting and drop the queued keystrokes without replaying."},
