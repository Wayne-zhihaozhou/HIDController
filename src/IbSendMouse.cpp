...
// 模拟鼠标移动（支持绝对和相对模式）
bool WINAPI Send::IbSendMouseMove(uint32_t x, uint32_t y, Send::MoveMode mode) {
	// 构造鼠标移动事件
	INPUT input{
		.type = INPUT_MOUSE,
		.mi {
...
// 模拟一次鼠标点击（按下 + 抬起）
bool WINAPI Send::IbSendMouseClick(Send::MouseButton button) {
	// 初始化按下和抬起事件
	INPUT inputs[2];
	inputs[0] = inputs[1] = {
		.type = INPUT_MOUSE,
...
// 模拟鼠标滚轮滚动
bool WINAPI Send::IbSendMouseWheel(int32_t movement) {
	// 构造滚轮事件
	INPUT input{
		.type = INPUT_MOUSE,
		.mi {
